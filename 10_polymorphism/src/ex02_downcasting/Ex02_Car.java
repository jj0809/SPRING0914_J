...
class EV extends Car{
	...
	public void charge(int battery) {
		this.battery += battery;
		System.out.println(getModel() + "의 배터리: " + this.battery); //모델명 확인: getModel 메소드 활용
	}
...
public class Ex02_Car {
	public static void main(String[] args) {
		...
		motors[1] = new Hybrid("그랜저", 40, 30);
	/*	motors[0].drive();
		motors[1].drive();
		((EV)motors[0]).charge(30);
		((Hybrid)motors[1]).addOil(70);
		*/
		//향상 for문: for (객체: 배열)
		//다운캐스팅을 확인할 때는 제일 밑 후손부터 확인해야함
		for (Car car : motors) {
			car.drive();
			if (car instanceof Hybrid) {
				((Hybrid)car).addOil(80);
